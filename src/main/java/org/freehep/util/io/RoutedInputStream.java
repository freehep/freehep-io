...
/**
 * The RoutedInputStream allows the user to add a listener
 * for a certain delimited portion of the main inputstream.
 * This portion is marked by a start and end marker.
 * The end marker can be null, in which case the portion
 * runs from the start marker to the end of the main inputstream.
 * The listener is informed via a route (partial inputstream)
 * when input is available.
 * The new routed inputstream (route) is supposed to be
 * read to the end or closed, after which the main
 * inputstream should be read again.
 * Closing a route will not close the original stream,
 * but will discard any bytes up to and including the end marker.
 * Returning from a route without reading until the end of
 * the route means that the remaining bytes are still available
 * on the main stream.
 * Multiple routes can be added, as long as they have
 * different start sequences.
 * Start sequences such as "StartA, StartB, StartEmpty"
 * are allowed, but "Start, StartOther" are not since they
 * overlap.
 * Start and End markers can be the same.
 *
 ...
 * @version $Id: src/main/java/org/freehep/util/io/RoutedInputStream.java b2aff02d4920 2005/11/18 22:58:46 duns $
 */
public class RoutedInputStream extends InputStream {
    ...
    private static final int ROUTEFOUND  = 1;  // found a route, but need to still return buffer
    private static final int ROUTEINFORM = 2;  // buffer returned, need to inform routelistener
    private static final int ROUTED      = 3;  // the main stream is now routed to the route
    private static final int CLOSING     = 4;  // the underlying stream is closed, but buffer need to be emptied
    ...
    /**
     * Returns the next byte on this stream, however if a start marker is
     * found, the associated route listener is called, which should take over
     * reading the stream. In this case this method will, after the route is
     * finished reading and closed, return the first byte after the end marker.
     * This of course unless that byte is part of the next start marker.
     */
        ...
        NEWSTATE: while (true) {
            switch(state) {
                ...
                case UNROUTED:
                    ...
                    // the buffer is one byte longer than the longest start marker, so even
                    // if we find that marker we can still return the byte just in front of it.
                    result = buffer[sob];
                ...
                case ROUTEINFORM:
                    ...
                    // next call will generate callbacks to this read method in state ROUTED.
                    ...
                    // FIXME: we need an UNROUTING here which just returns the buffer, but does not refill it, in case the reads would block...
     ...
     * The strings are converted according to the default encoding to start and
     * end markers (byte[]).
     */
    ...
    /**
     * Checks if cmp is equal to buf (with start at index) for the length
     * of cmp.
     */
     ...
     *
     * When you close the route, all bytes including the end marker
     * will be read/discarded before returning.
     *
     * If you just discard the Route, the underlying stream will still
     * return you all or part of the bytes of this route.
     *
     * If the end marker is set to null, the stream can be read until
    ...
    public class Route extends InputStream {
        ...
        public Route(byte[] start, byte[] end) {
            this.start = start;
            this.end = end;
            if (end != null) buffer = new byte[end.length];
            index = 0;
            closed = false;
        }
