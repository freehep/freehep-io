 ...
 *
 * <UL>
 * <LI><B>@ifdef property</B>, reads everything up to the next @statement if the
 *          property is defined.
 * <LI><B>@ifndef property</B>, reads everything up to the next @statement if the
 ...
 * @version $Id: src/main/java/org/freehep/util/io/ConditionalInputStream.java b2aff02d4920 2005/11/18 22:58:46 duns $
 */
public class ConditionalInputStream extends InputStream {
    ...
    public int read() throws IOException {
        ...
        if (b == '@') {
            if (escape) {
                ...
                if (keyword.equals("ifdef") || keyword.equals("ifndef")) {
                    ...
                    if (nesting <= 0) throw new RuntimeException("@else without corresponding @ifdef");
                    ok[nesting-1] = (nesting > 1 ? ok[nesting-2] : true) && !ok[nesting-1];
                    replaceBufferWithWhitespace(index);
                } else if (keyword.equals("endif")) {
                    // calculate inclusion based on ifdef nesting
                    if (nesting <= 0) throw new RuntimeException("@endif without corresponding @ifdef");
                    nesting--;
                    replaceBufferWithWhitespace(index);
                } else {
                    // not an known @
        ...
        if ((nesting > 0) && !ok[nesting-1]) {
            if (!Character.isWhitespace((char)b)) b = ' ';
        }
        return b & 0x00FF;
    }
